export const PRACTICE_IMPORT_BATCH_SIZE = 20
...
TASK: Generate exactly ${PRACTICE_IMPORT_BATCH_SIZE} MCQs for ONE subtopic. Output ONLY valid JSON (no markdown fences, no commentary).
...
RULES:
- Exactly ${PRACTICE_IMPORT_BATCH_SIZE} questions in the "questions" array
- 4 options A–D; exactly one correct (correctOption: "A"|"B"|"C"|"D")
- questionNumber: 1 through ${PRACTICE_IMPORT_BATCH_SIZE} on each item
...
Before output, self-check: ${PRACTICE_IMPORT_BATCH_SIZE} questions; questionNumber 1–${PRACTICE_IMPORT_BATCH_SIZE}; same sectionId and subtopicSlug on every row; each explanation has full option breakdown.
